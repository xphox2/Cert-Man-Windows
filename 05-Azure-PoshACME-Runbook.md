...
```
 Azure Automation runbook (scheduled daily/weekly)
        │  Posh-ACME
        ▼
   Let's Encrypt  ◄── DNS-01 TXT written to ──►  Azure DNS zone
        │
        ▼
   Cert issued ──► uploaded to Azure Key Vault
        │
        ▼
   App Service auto-imports from Key Vault (≤24h, no downtime)
        │
        ▼
   Posh-ACME state persisted to Azure Storage (so renewals are stateful)
```
