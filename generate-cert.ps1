...
function New-ManualVerifyScripts {
    # Generate the win-acme DNS validation create/delete scripts used by the one-time
    # "Manual + auto-verify" method. The CREATE script displays the TXT record win-acme
    # asks for, then polls authoritative + public DNS and only exits 0 once the record is
    # truly visible - so win-acme never submits to Let's Encrypt prematurely (which would
    # invalidate the token and force you to edit DNS again). On timeout it EXTENDS rather
    # than fails, guaranteeing the token is never thrown away.
    param([int]$WaitMinutes = 60, [int]$PollSeconds = 20, [int]$QuorumPublic = 3)
    $create = Join-Path $WinAcmePath 'manual-dns-verify.ps1'
    $delete = Join-Path $WinAcmePath 'manual-dns-cleanup.ps1'

    $body = @'
param([string]$RecordName, [string]$Token)
# Auto-generated by generate-cert.ps1 - win-acme DNS-01 create/verify hook (manual, one-time).
# win-acme BLOCKS until this exits 0, so we poll until the record is globally visible.
$ErrorActionPreference = 'SilentlyContinue'
$WaitMinutes  = __WAIT__
$PollSeconds  = __POLL__
$QuorumPublic = __QUORUM__
...
function Get-Auth {
    ...
    # then resolve those nameservers to IPs (this is what Let's Encrypt itself will query).
    param($Name)
    $labels = $Name.Split('.')
    for ($i = 0; $i -lt $labels.Count - 1; $i++) {
        ...
        if ($ns.Count) {
            ...
            return [pscustomobject]@{ Zone = $zone; Servers = @($ns); Ips = @($ips | Select-Object -Unique) }
        }
    }
    return [pscustomobject]@{ Zone = $null; Servers = @(); Ips = @() }
}
...
Write-Host ''
Write-Host '  Add it ONCE. This will keep checking authoritative + public DNS and continue only' -ForegroundColor Gray
Write-Host '  when the record is visible, so the request will not fail and the value never changes.' -ForegroundColor Gray
Write-Host '  Leave this window open (Ctrl+C aborts without issuing).' -ForegroundColor Gray
Write-Host ''

# Also persist to a file in case the console scrolls.
...
if ($auth.Zone) { Write-Host ('  Authoritative zone: {0}  ({1} nameserver IP(s) detected)' -f $auth.Zone, $auth.Ips.Count) -ForegroundColor DarkGray }
else { Write-Host '  Could not detect authoritative nameservers yet; relying on public resolvers for now.' -ForegroundColor DarkGray }
Write-Host ''
...
while ($true) {
    ...
    if (-not $auth.Ips.Count) { $auth = Get-Auth $RecordName }   # NS discovery may succeed once delegation settles

    # Authoritative check - this is exactly what Let's Encrypt resolves, so it is the real gate.
    ...
    # Public resolver check - explicitly online (non-local) DNS, which may lag due to caching.
    $pubHit = 0
    ...
    Write-Host ('  [{0}] try {1,-3} {2}; public {3}/{4} see it...' -f $stamp, $attempt, $aTxt, $pubHit, $Resolvers.Count) -ForegroundColor DarkGray
    ...
    if ((Get-Date) -gt $deadline) {
        Write-Host ''
        Write-Host ('  Still not visible after {0} min. NOT failing - will keep waiting.' -f $WaitMinutes) -ForegroundColor Yellow
        Write-Host ('  Re-check the record (exact Name/Value above, TXT type, no extra quotes). Saved to: {0}' -f $todo) -ForegroundColor Yellow
        $deadline = (Get-Date).AddMinutes($WaitMinutes)   # EXTEND, never fail - the token must not be thrown away
    }
    Start-Sleep -Seconds $PollSeconds
}
'@
    $body = $body.Replace('__WAIT__', "$WaitMinutes").Replace('__POLL__', "$PollSeconds").Replace('__QUORUM__', "$QuorumPublic")
    Set-Content -Path $create -Value $body -Encoding UTF8

    $del = @'
param([string]$RecordName)
# Manual DNS: nothing to delete automatically - just remind the operator.
Write-Host ''
Write-Host ('  You may now remove the one-time TXT record: {0}' -f $RecordName) -ForegroundColor DarkGray
exit 0
'@
    Set-Content -Path $delete -Value $del -Encoding UTF8
    return [pscustomobject]@{ Create = $create; Delete = $delete }
}
...
function Get-DnsValidation {
    ...
    switch (Read-Host '   Choose 1-7') {
        ...
        '7' {
            ...
            $sc = New-ManualVerifyScripts -WaitMinutes $wait
            $vargs = @('--validationmode', 'dns-01', '--validation', 'script',
                '--dnscreatescript', $sc.Create, '--dnscreatescriptarguments', '{RecordName} {Token}',
                '--dnsdeletescript', $sc.Delete, '--dnsdeletescriptarguments', '{RecordName}')
